...
module.exports = React.createClass({
    ...
    render(){
        let [start, end, format, condition, text] = [this.state.start, this.state.end, 'YYYY-MM-DD', this.props.condition, this.state.text];
        return (
            <div>
                <div className={this.state.up ? 'animate_up condition' : 'animate_down condition'}>
                    <Row>
                        <Col span={9} style={{paddingLeft: '30px'}}>
                            店仓: <span style={{textAlign: 'left'}} className="store"><span data-type={condition[0]}
                                                                                          onClick={this.handleClick}
                                                                                          className="common">{text['store'].join(' ')}</span><Icon
                            type="right"/></span>
                        </Col>
                        {/*<Col span={1} />*/}
                        {/*<Col span={9} style={{paddingLeft: '30px'}}>*/}
                        ...
                        <Col className="datetime" span={9} style={{textAlign: 'right'}}>
                            日期: <DatePicker defaultValue={moment(start, format)} format={format}
                                            style={{marginLeft: '10px'}} onChange={this.onStartChange}/> - <DatePicker
                            defaultValue={moment(end, format)} format={format} onChange={this.onEndChange}/>
                        </Col>
                        <Col span={6} style={{paddingLeft: '30px'}}>
                            <p className="pick_up"><span className="up_down" onClick={this.slideUp}>收起条件<Icon
                                className="up" type={this.state.up ? 'down' : 'up'}/></span><Icon className="search"
                                                                                                  type="search"
                                                                                                  onClick={this.search_product_code}/>
                            </p>
                        </Col>
                    </Row>
                    <Row>
                        <Col span={9} style={{paddingLeft: '30px'}}>
                            分类: <span style={{textAlign: 'left'}} className="store"><span className="common" data-type={condition[1]}
                                                                   onClick={this.handleClick}>{text['kind'].join(' ')}</span><Icon
                            type="right"/></span>
                        </Col>
                        {/*<Col span={1} />*/}
                        <Col span={9} style={{paddingLeft: '8px'}}>
                            <span className="range-before">排名范围:</span><span className="range"><input
                            type="text"/> - <input type="text"/></span>
                        </Col>
                        <Col span={6} style={{paddingLeft: '30px'}}>
