...
module.exports = React.createClass({
    ...
    onChangeStatus(res, type){
        ...
        if(type == 'store'){
            ...
            if(sessionStorage.getItem('5_1_filter_condition')){
                filter_condition = JSON.parse(sessionStorage.getItem('5_1_filter_condition'))
            }
    ...
    table(res, array, render_data, head_columns, head_columns_key){
        ...
        table.footer = function () {
            if(res.data.records.length){
                return <div>合计 |<span style={{display: 'inline-block', margin: '0 20px'}}>销售数量: {res.data.sale_count_sum}</span> <span>销售金额: {res.data.sale_amount_sum}</span></div>;
            }else{
                return '';
            }
        };
    ...
    changePage(page){
        ...
        sessionStorage.setItem('5_1_current', page);
        sessionStorage.setItem('5_1_obj', JSON.stringify(obj));
        SaleDayAction.tables(obj, 'table')
    },
    onShowSizeChange(current, pageSize){
        ...
        sessionStorage.setItem('5_1_pageSize', pageSize);
        sessionStorage.setItem('5_1_obj', JSON.stringify(obj));
        SaleDayAction.tables(obj, 'table')
    },
    getDefaultProps(){
    ...
    getInitialState(){
        var start_date = sessionStorage.getItem('5_1_start_date') ? sessionStorage.getItem('5_1_start_date') : '';
        var end_date = sessionStorage.getItem('5_1_end_date') ? sessionStorage.getItem('5_1_end_date') : '';
        var pageSize = sessionStorage.getItem('5_1_pageSize') ? JSON.parse(sessionStorage.getItem('5_1_pageSize')) : 20;
        var current = sessionStorage.getItem('5_1_current') ? JSON.parse(sessionStorage.getItem('5_1_current_store')) : 1;
        var product_code = sessionStorage.getItem('5_1_product_code') ? sessionStorage.getItem('5_1_product_code') : '';
        return {
            height: 0, //condition
            height1: 0, //merge
    ...
    componentWillMount(){
      ...
      if(sessionStorage.getItem('5_1_start_date')){
          date_start = sessionStorage.getItem('5_1_start_date')
      }
      if(sessionStorage.getItem('5_1_end_date')){
          date_end = sessionStorage.getItem('5_1_end_date')
      }
      this.setState({
    ...
    componentDidMount(){
        ...
        if(sessionStorage.getItem('5_1_obj')){
            var obj = JSON.parse(sessionStorage.getItem('5_1_obj'));
            SaleDayAction.tables(obj,'table');
        }
        var str = 'th.sale_count,th.real_amount,th.cut_rate,th.cut_amount,th.sale_amount';
        this.click_sort(str);
    },
    onStartChange(date, dateString){
        sessionStorage.setItem('5_1_start_date', dateString)
        this.setState({
            start_date: dateString
        })
    },
    onEndChange(date, dateString){
        sessionStorage.setItem('5_1_end_date', dateString)
        this.setState({
            end_date: dateString
        })
    },
                    ...
                    <Row>
                        <Col span={12}>
                          <div className="common">
                            <span className="first" data-type="store" onClick={this.handleClick}>店仓: </span><span className="second" data-type="store" onClick={this.handleClick}>{filter_condition.store.text}</span><Icon type="right"/>
                          </div>
                        </Col>
    ...
    sumLevelRadio(e){
        ...
        sessionStorage.setItem('5_1_filter_condition', JSON.stringify(filter_condition))
        this.setState({
            filter_condition: filter_condition
        })
    },
    ...
    search(){
        ...
        sessionStorage.setItem('5_1_obj', JSON.stringify(result));
        SaleDayAction.tables(result, 'table');
    },
    expandCondition(){
    ...
    handleProductCode(e){
        sessionStorage.setItem('5_1_product_code', e.target.value);
        this.setState({
    ...
    mergeResult(){
        ...
        if(filter_condition.store.text){
            str += '店铺范围: ' + filter_condition.store.text + '; ';
        }
