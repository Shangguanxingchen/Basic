...
var MemberChart1 = React.createClass({
  ...
  componentDidMount(){
    this.chart = echarts.init(this.refs.member3, 'dark');
    this.refreshChart();
  },
  componentDidUpdate(){
    this.refreshChart();
  ...
  renderChart(){
    ...
    var myChart;
    if(typeof myCharts['member3'] == 'undefined'){
      myChart = echarts.init(document.getElementById('member3'))
      myCharts.member3 = myChart
    }else{
      myChart = myCharts['member3']
    }
    // var myChart = echarts.init(document.getElementById('member3'));
  ...
  refreshChart(){
	...
	render(){
		return(
      <div ref = "member3" id = "member3"></div>  
		)
	}
})
