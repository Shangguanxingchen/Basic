...
var EditorChart = React.createClass({
  ...
  componentDidMount(){
    this.chart = echarts.init(this.refs.Inditor1, 'dark');
    this.refreshChart();
  },
  componentDidUpdate(){
    this.refreshChart();
  ...
  renderChart(){
    var option;
    var myChart;
    if(typeof myCharts['Inditor1'] == 'undefined'){
      myChart = echarts.init(document.getElementById('Inditor1'))
      myCharts.Inditor3 = myChart
    }else{
      myChart = myCharts['Inditor1']
    }
    // var myChart = echarts.init(document.getElementById('Inditor1'));
    var value = this.props.series;
    var sale_amount = value.sold_amount;
    var zhibiao = value.day_indicator;
    // var num = (sale_amount/zhibiao)*100;
    var num;
    if(sale_amount == 0){
        num = 0;
    }else if(zhibiao == 0){
  ...
  refreshChart(){
	...
	render(){
		return(
			<div ref = "Inditor1" id = "Inditor1"></div>	
		)
	}
})
