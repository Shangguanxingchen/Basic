...
var MemberChart1 = React.createClass({
  ...
  componentDidMount(){
    this.chart = echarts.init(this.refs.member1, 'dark');
    this.refreshChart();
  },
  componentDidUpdate(){
    this.refreshChart();
  ...
  renderChart(){
    ...
    var myChart;
    if(typeof myCharts['member1'] == 'undefined'){
      myChart = echarts.init(document.getElementById('member1'))
      myCharts.member1 = myChart
    }else{
      myChart = myCharts['member1']
    }
  ...
  refreshChart(){
	...
	render(){
		return(
      <div ref = "member1" id = "member1"></div>  
		)
	}
})
