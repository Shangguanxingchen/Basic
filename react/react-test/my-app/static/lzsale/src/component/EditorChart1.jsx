...
var EditorChart = React.createClass({
  ...
  componentDidMount(){
    this.chart = echarts.init(this.refs.editor1, 'dark');
    this.refreshChart();
  },
  componentDidUpdate(){
    this.refreshChart();
  ...
  renderChart(){
    ...
    var myChart;
    if(typeof myCharts['editor1'] == 'undefined'){
      myChart = echarts.init(document.getElementById('editor1'))
      myCharts.editor1 = myChart
    }else{
      myChart = myCharts['editor1']
    }
    // var myChart = echarts.init(document.getElementById('editor1'));
    var names = [],amount;
  ...
  refreshChart(){
    ...
    // var value = this.props.series;
		...
		return(
      <div ref = "editor1" id = "editor1"></div>  
		)
	}
})
