...
import DayAction from '../actions/DayAction.jsx';
import DayStore from '../stores/DayStore.jsx'
...
var Day = React.createClass({
  ...
  onStatusChange(data){
    this.setState({
      ...
      month_indicator:data.month_indicator,
    })
  ...
  handleChange(e){
    ...
    var {month_indicator} = this.state;
    ...
    // 保存验证
    dayLine[index]['rate'] = (dayLine[index].day_indicator/month_indicator*100).toFixed(2) + '%';
    var sum = 0;
    ...
    var indicator_unassign = (month_indicator - sum).toFixed(2);
    this.setState({
      dayLine:dayLine,
  ...
  onClear(e){
    var {month_indicator} = this.state;
    var {dayLine} = this.state;
    var old_value = $(e.target).parent().parent().find('input')[0].value;
    $(e.target).parent().parent().find('input')[0].value = ''; 
    
    var _index = $(e.target).parents('li').find('input').attr('data-index');
    dayLine[_index]['day_indicator'] = 0;
    dayLine[_index]['rate'] = '0.00%';
    ...
    var indicator_unassign = (month_indicator - new_sum).toFixed(2);
        ...
        if(index < parseInt(indexs)){
          ...
          return(
            <div key = {i}>
              <ul key = {i} className= 'content'>
                <li>{item.day}</li>
                <li style={{width:'40%',padding:'10px',position:'relative'}}>
                  ...
                  <div className="clear" data-index = {i} onClick= {self.onClear}><img data-index = {i} src="static/lzsale/src/img/clear.png"/></div>
                </li>
                <li>{item.rate}</li>
              </ul>
            </div>
    ...
    return(
      <div>
        ...
        <div className="btn_message Close">
          <p style={{padding:'20px'}}>提示</p>
          <p className="num">还有<span style={{color:'#FF3510'}}>未分配</span>指标不可保存</p>
          <p className="title">分配的指标量需大于或等于月指标量</p>
          <p className="btn">
            <span onClick={this.handleKnow}>确认</span></p>
        </div>
        <div className="btn_message giveUp">
          <p style={{padding:'20px'}}>提示</p>
          <p className="title" style={{padding:'30px'}}>指标未保存，是否放弃对当前指标的编辑修改</p>
          <div className="Btn">
            <button onClick={this.handleGiveup} style={{marginRight:'1px solid #d5d5d5'}}><a href ="#/indicator">放弃</a></button>
            <button onClick={this.handleKnow} style={{borderRight:'1px solid #d5d5d5'}}>取消</button>
          </div>
        </div>
        ...
        <Row type = "flex" className="nav_header">
          {/*<Col span = {2}>
            <a href ="#/indicator"><img src="static/indicator/src/img/left.png" className="img"/></a>
          </Col>*/}
          <Col span = {24}>
            <span style={{color:'#788195',fontFamily:'serif'}}>门店日指标设置</span>
          </Col>
        </Row>
        <Row type = "flex" className = "content_1 content_2 BG">
          ...
          <Col span = {16} className="right">
            <p style={{padding:'14px',textAlign:'center',borderBottom:'1px solid #d5d5d5'}}>月指标量 <span style={{fontSize:'20px'}}>¥{day.month_indicator}</span>
            <span className="hide_no">/<span style={{color:'red',fontSize:'20px'}}>未分配¥{indicator_unassign}</span></span>
            <span className="hide_yes">/<span style={{color:'red',fontSize:'20px'}}>超出¥{indicator_unassign}</span></span>
            </p>
            <ul>
              <li>日期</li>
            ...
            <div className="btn">
                <ul>
                  ...
                  <a><li onClick={self.handleClose}>取消</li></a>
                </ul>
              </div>
          </Col> 
        </Row>
