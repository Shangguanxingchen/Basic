...
var EditorChart = React.createClass({
  ...
  componentDidMount(){
    this.chart = echarts.init(this.refs.editor4, 'dark');
    this.refreshChart();
  },
  componentDidUpdate(){
    this.refreshChart();
  ...
  renderChart(){
    ...
    var myChart;
    if(typeof myCharts['editor4'] == 'undefined'){
      myChart = echarts.init(document.getElementById('editor4'))
      myCharts.editor4 = myChart
    }else{
      myChart = myCharts['editor4']
    }
    // var myChart = echarts.init(document.getElementById('editor4'));
    var names = [],amount;
  ...
  refreshChart(){
    ...
    // var value = this.props.series;
		...
		return(
      <div ref = "editor4" id = "editor4"></div>  
		)
	}
})
