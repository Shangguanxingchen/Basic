...
var EditorChart = React.createClass({
  ...
  componentDidMount(){
    this.chart = echarts.init(this.refs.editor5, 'dark');
    this.refreshChart();
  },
  componentDidUpdate(){
    this.refreshChart();
  ...
  renderChart(){
    ...
    var myChart;
    if(typeof myCharts['editor5'] == 'undefined'){
      myChart = echarts.init(document.getElementById('editor5'))
      myCharts.editor5 = myChart
    }else{
      myChart = myCharts['editor5']
    }
    // var myChart = echarts.init(document.getElementById('editor5'));
    var names = [],amount;
  ...
  refreshChart(){
    ...
    // var value = this.props.series;
		...
		return(
      <div ref = "editor5" id = "editor5"></div>  
		)
	}
})
