...
var EditorChart = React.createClass({
  ...
  componentDidMount(){
    this.chart = echarts.init(this.refs.editor2, 'dark');
    this.refreshChart();
  },
  componentDidUpdate(){
    this.refreshChart();
  ...
  renderChart(){
    ...
    var myChart;
    if(typeof myCharts['editor2'] == 'undefined'){
      myChart = echarts.init(document.getElementById('editor2'))
      myCharts.editor2 = myChart
    }else{
      myChart = myCharts['editor2']
    }
    // var myChart = echarts.init(document.getElementById('editor2'));
    var names = [],amount;
  ...
  refreshChart(){
    ...
    // var value = this.props.series;
		...
		return(
      <div ref = "editor2" id = "editor2"></div>  
		)
	}
})
