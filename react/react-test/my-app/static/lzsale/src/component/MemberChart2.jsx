...
var MemberChart1 = React.createClass({
  ...
  componentDidMount(){
    this.chart = echarts.init(this.refs.member2, 'dark');
    this.refreshChart();
  },
  componentDidUpdate(){
    this.refreshChart();
  ...
  renderChart(){
    ...
    var myChart;
    if(typeof myCharts['member2'] == 'undefined'){
      myChart = echarts.init(document.getElementById('member2'))
      myCharts.member2 = myChart
    }else{
      myChart = myCharts['member2']
    }
  ...
  refreshChart(){
	...
	render(){
		return(
      <div ref = "member2" id = "member2"></div>  
		)
	}
})
