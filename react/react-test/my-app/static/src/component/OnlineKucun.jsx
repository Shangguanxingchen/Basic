...
module.exports = React.createClass({
    ...
    table(res, array, render_data, head_columns, head_columns_key){
        ...
        table.footer = function () {
            if(res.data.records.length){
                return <div>合计 |<span style={{display: 'inline-block', margin: '0 20px'}}>金额总数: {res.data.sale_count_sum}</span> </div>;
            }else{
                return '';
            }
        };
    ...
    componentWillMount(){
      ...
      for (var i = 0; i < 7; i++) {
        var G_month =  String(myDate.getMonth()+1).length == 1 ? (myDate.getMonth()+1) : myDate.getMonth()+1;
        var G_day = String(myDate.getDate()).length == 1 ? (myDate.getDate()) : myDate.getDate();
          dateTemp = G_month+"-"+ G_day;
          dateArray.push(dateTemp);
      }
      let [date_start, date_end] = [Year + '-' + dateArray[0], Year + '-' + Month + '-' + Day];
        ...
        return (
            <div>
                <div className={after_search ? "hide condition" : "show condition"}>
                    ...
                    <Row>
                        <Col span={12}>
                            <div className="common checkbox_single" style={{marginLeft:'47px'}}>
                                <Checkbox onChange={this.handleTheMount} checked={with_the_mount}>含在途</Checkbox>
                            </div>
                        </Col>
                        <Col span={12}>
                ...
                <div className={has_data ? 'table show' : 'table hide'}>
                    <Table scroll={{y: y, x: x}} bordered pagination={pagination} dataSource={table.dataSource} columns={table.columns} footer={table.footer}/>
                </div>
                {/*黑色背景弹框*/}
    ...
    ajax(){
        ...
        var shop_id = this.getShopId('shop_id=');
        var {conditionContent} = this.props;
        var {pagination, with_the_mount, start_date, end_date, product_code, filter_condition} = this.state;
    ...
    sort(type){
        ...
        sessionStorage.setItem('ajax_online_kucun', JSON.stringify(obj));
        StoreXiaoCunAction1.table(obj, 'table');
    },
    click_sort(str){
        var _this = this;
    ...
    mergeResult(){
        var {product_code, filter_condition, start_date, end_date,with_the_mount} = this.state;
        var str = '', kind_i = 0;
        str += '日期: ' + start_date + '至' + end_date + '; ';
        if(product_code.trim()){
            str += '商品编码: ' + product_code + '; ';
        }
        if(Number(with_the_mount) == 1){
            str += '含在途';
        }
