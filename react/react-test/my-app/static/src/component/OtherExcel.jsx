...
var OtherExcelStore = require('../store/OtherExcelStore');
var OtherExcelAction = require('../actions/OtherExcelAction');

import {Row, Col, DatePicker, Icon, Checkbox, Radio, Input, Table} from 'antd';
import moment from 'moment';

module.exports = React.createClass({
  mixins: [Reflux.listenTo(OtherExcelStore, 'onStatusChange')],
  onStatusChange(data){
      ...
      if(data.condition == 'kind'){
          ...
          var [total,total_amount, total_qty, head_columns, head_columns_key, dataSource, render_data]
              =
              [data.list.page_count,data.list.sale_sum, data.list.page_count, [], [], data.list.records, []];
    ...
    componentDidMount(){
        ...
        OtherExcelAction.high(condition[1]);
        if(sessionStorage.getItem('ajax_6')){
            OtherExcelAction.table(JSON.parse(sessionStorage.getItem('ajax_6')));
        }
    ...
    common_ajax(page, start, end, pageSize){
      ...
      var [view_type, brand_ids, series_ids, limit, current_page, start_date, end_date, product_name]
          =
      ...
      for(var i in checked['high']){
          if(checked['high'][i]){
              view_type = list['high'][i].type_code;
          }
      }
    ...
    render(){
        ...
        return (
            <div>
                <div style={{height: '60px'}} className='condition'>
                    <Row>
                        ...
                        <Col span={9} style={{paddingLeft: '30px'}}>
                            {/*分类: <span className="inner_kind"><span onClick={this.handleClick} data-type={condition[0]} className="common">{text['kind'].join(' ')}</span><Icon type="right" /></span>*/}
                        </Col>
                        <Col span={6} style={{paddingLeft: '30px'}}>
                            <a className="high_select" data-type={condition[1]} onClick={this.handleClick}>高级选项</a><Icon className="search" type="search" onClick={this.search_product_code} style={{left:"25px"}}/>
                        </Col>
                    </Row>
                </div>
                <div id="width" className="table_wrap">
                  <div className="table">
