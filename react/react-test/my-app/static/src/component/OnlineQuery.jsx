...
import OnlineQueryStore from '../store/OnlineQueryStore'

var Reflux = require('reflux');
var $ = require('jquery');

module.exports = React.createClass({
    mixins: [Reflux.listenTo(OnlineQueryStore, 'onStatusChange')],
    onStatusChange(data){
        var [list, checked, text] = [this.state.list, this.state.checked, this.state.text];
        if(data.condition == 'brand' || data.condition == 'series'){
    ...
    componentDidMount(){
        ...
        OnlineQueryAction.brand(condition[0]);
        OnlineQueryAction.series(condition[1]);
    },
    handleClick(e){
        var [type, titles]
            =
    ...
    common_ajax(page){
        ...
        var [brand_ids, series_ids, limit, current_page, product_name]
            =
