...
var NowSalesStore = require('../store/NowSalesStore');
...
module.exports = React.createClass({
  mixins: [Reflux.listenTo(NowSalesStore, 'onStatusChange')],
  onStatusChange(data){
      ...
      if(data.condition == 'kind'){
          ...
          var [total,total_amount, total_qty, head_columns, head_columns_key, dataSource, render_data]
              =
              [data.list.record_count,data.list.sale_sum, data.list.record_count, [], [], data.list.records, []];
    ...
    componentDidMount(){
        ...
        NowSalesAction.kind(condition[0]);
        NowSalesAction.high(condition[1]);
        if(sessionStorage.getItem('ajax_6')){
            NowSalesAction.table(JSON.parse(sessionStorage.getItem('ajax_6')));
        }
    },
    ...
    common_ajax(page, start, end, pageSize){
      ...
      var [view_type_list, category_ids, series_ids, limit, current_page, start_date, end_date, product_name]
          =
    ...
    table(){
        ...
        table.footer = function () {
            if(table_data.length){
                return <div> <span style={{display: 'inline-block', marginRight: '20px'}}>总数量: {total_qty}</span><span>总销售金额: {total_amount}</span></div>;
            }else{
                return '';
            }
        }
